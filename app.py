    ...
    if not SHAP_AVAILABLE:
        ...
        with tab_gemini:
            ...
            st.markdown(
                ...
                "narratives using Google Gemini. If no API key is set, a "
                "rule-based fallback generates the explanation."
            )

            gemini_key = st.text_input(
                "Gemini API Key (optional — leave blank to use env var or fallback)",
                type="password",
            ...
            if st.button("Generate Business Explanation", key="gemini_btn"):
                ...
                use_gemini = False
                api_key = gemini_key.strip() if gemini_key else os.environ.get("GEMINI_API_KEY", "")
                ...
                if use_gemini:
                    ...
                    st.caption(
                        "💡 Rule-based explanation (set GEMINI_API_KEY or enter key above "
